...
from metrics.views import getStory, getSprint, getRelease
...
def updateStory(this, that, session):
    print "Updating " + this.rallyNumber
...
response = rally.get('UserStory',query=q,fetch="FormattedID,Name,PlanEstimate,c_BusinessValueBV,ScheduleStatePrefix,Package,c_SolutionSize,c_Stakeholders,Iteration,Release,Tags,RevisionHistory",order="FormattedID")
...
for story in response:
    ...
    if this == None:
      # Create new instance
        createStory(story, session, solSize)
    else:
